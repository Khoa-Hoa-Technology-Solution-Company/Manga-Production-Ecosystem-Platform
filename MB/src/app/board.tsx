...
function EditorialBoardScreen() {
  ...
  const submitMeeting = async () => {
    ...
    if (!meetingTitle.trim() || Number.isNaN(date.getTime()) || !meetingSeriesIds.length || !participantIds.length) {
  ...
  const createRubric = async () => {
    ...
    if (!rubricName.trim() || !criteriaItems.length || savingRubric) return;
